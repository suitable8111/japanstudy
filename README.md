...
[홈 화면]
   ...
   │                        ├── 공부하기 → [음절 학습 화면]
   │                        └── 써보기 → [필기 연습 화면] (ML Kit 인식 + 정답 판정)
   ├── 1단계: 단어 외우기 → [모드 선택(공부하기/써보기)] → [레벨 선택(전체/N5~N1)] → [단어 학습 화면 or 단어 써보기 화면]
...
- [x] **히라가나/가타카나 써보기**: 청음 46자 순서대로/랜덤 필기 연습 + 이전/다음 네비게이션 (CustomPainter 캔버스)
- [x] **써보기 (발음만) 모드**: 글자를 가리고 소리만 듣고 쓰는 고급 연습 모드 — 정답 보기·다시 듣기 버튼 포함
...
### Phase 8 - 게이미피케이션 & 리텐션 강화 (완료)

- [x] **연속 학습 스트릭(Streak) 시스템**: 홈 화면 불꽃 아이콘 + pulse 애니메이션, 마일스톤(3/7/14/30/50/100일) 달성 시 축하 다이얼로그 1회 표시
- [x] **업적 및 배지 시스템**: 프로필 화면 4×4 배지 그리드 — 스트릭/학습/퀴즈/세션 카테고리 16개 배지, 달성/미달성 시각 구분
- [x] **앱 테마 커스텀**: 설정 화면에서 5가지 색상 테마(보라/파랑/청록/장미/주황) 선택 — 홈·드로어·프로필 헤더 그라디언트 실시간 반영

### Phase 9 - ML Kit 필기 인식 (완료)

- [x] **단어 써보기 모드**: 한국어 뜻(+히라가나 힌트)을 보고 일본어 단어 전체를 캔버스에 필기 → Google ML Kit Digital Ink Recognition 자동 판정 (상위 10개 후보 매칭)
- [x] **히라가나/가타카나 써보기 ML Kit 적용**: 기존 따라 쓰기 모드에 ML Kit 인식 추가 — "확인" 버튼으로 필기 제출 → 정답/오답 즉시 표시, `both` 모드는 히라가나·가타카나 중 하나만 맞아도 정답 처리
- [x] **타임스탬프 캔버스 (`RecognitionCanvas`)**: ML Kit 제출에 필요한 `(x, y, t)` 포인트 저장 위젯 신규 제작
- [x] **모델 관리**: 일본어 모델 자동 다운로드 (WiFi/모바일 데이터 모두 지원), 다운로드 배너 + 실패 시 재시도 버튼
